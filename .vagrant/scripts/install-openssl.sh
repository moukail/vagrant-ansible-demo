...
if [ -x "$(command -v dnf)" ]; then
    dnf update -yq
    dnf install -yq wget gcc make zlib-devel
elif [ -x "$(command -v apt)" ]; then
    apt update -yq
    apt install -yq wget gcc make zlib1g-dev
else
...
if [ "$(openssl version | awk '{print $2}')" == "3.5.4" ]; then
    ...
    ./Configure --prefix=/usr --openssldir=/etc/ssl shared zlib enable-md2
